...
# formula=pd.read_csv("/work/mroitegui/Superconductors/data/Supercon_data.csv")
...
# nagasawa = (1/constants.Boltzmann)*(((constants.Planck)**2)/(2*np.pi*2*matmas))*2*np.pi*(1/(maradius)**2)*((2*eleclist)/(np.log(2*eleclist)))
...
## EXTRACT REAL Y_TEST ##
# #######
# X_test2 = Xn[21267:size_dataset,:]
# y_test2 = yn[21267:size_dataset]
X_test2 = Xn[21263:,:]
y_test2 = yn[21263:]
Xn = Xn[0:21263, :]
yn = yn[0:21263]
...
regr = xgboost.XGBRegressor(n_estimators=500, # 200
                                max_depth=7, # 7
                                eta=0.1, # 0.1 
                                # gamma = 0.5,
                                subsample=0.7, # 0.7
