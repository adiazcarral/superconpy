...
for superconductor in superconductors_list:
    ...
    if superconductorsdict[superconductor] <= 35:
            
            lowTt.append(superconductors_list.index(superconductor))
            lowTtname.append(superconductor) 
    else:
            highTt.append(superconductors_list.index(superconductor))
            highTtname.append(superconductor)       
...
lowTtcsv = formula.loc[lowTt] 
lowTtcsv['DOPPED']=lowTtname
lowTtcsv.to_csv('lowTt.csv', encoding='utf-8', index=False)

highTtcsv = formula.loc[highTt] 
highTtcsv['DOPPED']=highTtname
highTtcsv.to_csv('highTt.csv', encoding='utf-8', index=False)
